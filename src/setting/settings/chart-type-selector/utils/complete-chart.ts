...
const completeBackgroundColor = (
  webChart: ImmutableObject<IWebChart>,
): ImmutableObject<IWebChart> => {
  const background = webChart.background || DefaultBgColor;
  webChart = webChart.set("background", background);
  return webChart;
};

const DefaultGrid = getLineSymbol(0, DefaultGridColor, "esriSLSDash");

const completeAxes = (
  webChart: ImmutableObject<IWebChart>,
): ImmutableObject<IWebChart> => {
  if (!isXYChart(webChart?.series) && !isGaugeChart(webChart?.series)) {
    return webChart;
  }
