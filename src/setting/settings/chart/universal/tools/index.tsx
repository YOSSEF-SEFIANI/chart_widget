...
export const Tools = (props: ToolsProps): React.ReactElement => {
  ...
  const translate = hooks.useTranslation(
    jimuUiDefaultMessage,
    jimuCoreDefaultMessage,
  );
  const selectionLabel =
    type === "pieSeries"
      ? translate("selection")
      : `${translate("selection")} & ${translate("ZoomLabel")}`;
  ...
  return (
    ...
    >
      <SettingRow
        ...
        className="mt-4"
      >
        <Switch checked={cursorEnable} onChange={handleCursorEnableChange} />
      </SettingRow>
    </div>
  );
};
