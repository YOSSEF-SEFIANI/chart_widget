...
export const SerialSeriesSetting = (props: SerialSeriesSettingProps): React.ReactElement => {
  ...
  return (
    <div className='serial-series-setting w-100' role='group' aria-label={translate('series')}>
      ...
      <SettingRow className='mt-3' level={2} tag='label' label={translate('hideWidgetOnNoData')}>
        <Switch
          checked={options.hideOnNoData}
          onChange={(_, checked) => { onOptionsChange?.(options.set('hideOnNoData', checked)) }}
        />
      </SettingRow>
    </div>
  )
