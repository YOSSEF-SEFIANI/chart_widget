...
function WebChartComponent(props: WebChartComponentProps): React.ReactElement {
  ...
  const colorMatchAllowed = !isPieChart && propWebChart.colorMatch && (propWebChart?.series?.length === 1 || !!propWebChart?.series?.[0]?.query?.where)
  ...
  const webMapWebChart = React.useMemo(() => {
    ...
    // Le JSON montre "fillSymbol": { "color": "#5E8FD0" }.
    // Si on ne supprime pas ça, la palette UI est ignorée.
    if (isPieChart && series && series.length > 0) {
       series = series.map(s => {
         // .without("fillSymbol") supprime la définition de couleur unique
         // Cela oblige le graphique à utiliser 'customColors'
  ...
  }, [id, propWebChart, num, isPieChart, hasUiColors, uiCustomColors])
