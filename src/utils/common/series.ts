 ...
 * @param dataSourceId
 */
export const getFieldsAlias = (
  jimuFieldNames: string[] | ImmutableArray<string>,
  dataSourceId: string,
) => {
  return jimuFieldNames?.map((jimuFieldName) =>
    getFieldAlias(jimuFieldName, dataSourceId),
  );
};
...
export const getTemplateType = (
  webChart: IWebChart | ImmutableObject<IWebChart>,
  ...
  if (seriesType === "barSeries") {
    ...
    const prefix =
      webChart.stackedType === "sideBySide" ? "" : webChart.stackedType;
    type = suffix;
...
export const getOutStatisticAlias = (
  ...
  if (statisticType !== "no_aggregation") {
    const type =
      statisticType === "percentile-continuous" ? "median" : statisticType;
...
export const getDefaultValueFormat = (
  ...
  discreteForNumeric: boolean = false,
): CategoryFormatOptions | NumberFormatOptions | DateTimeFormatOptions => {
...
export const getSplitByFieldValues = (
  propSeries: ImmutableArray<WebChartSeries> | WebChartSeries[],
): Array<number | string> => {
  if (!propSeries?.length) return [];
  const series: WebChartSeries[] = (
    propSeries as ImmutableArray<WebChartSeries>
  ).asMutable
    ? (propSeries as ImmutableArray<WebChartSeries>).asMutable({ deep: true })
    : (propSeries as WebChartSeries[]);
...
export const createDefaultSerie = (
  ...
  if (type === "barSeries") {
    ...
    } = seriesProps as WebChartBarChartSeries;
    ...
    const {
      ...
      startAngle = 0,
      endAngle = 360,
    } = seriesProps as WebChartPieChartSeries;
    ...
    const { fillSymbol, binCount, overlays, dataTransformationType } =
      seriesProps as WebChartHistogramSeries;
    ...
    const { valueConversion, featureIndex = 0 } =
      seriesProps as WebChartGaugeSeries;
...
export const getUsedSeriesProps = (
  ...
  if (!defaultSerie) {
    ...
    const { hideOversizedStackedLabels, binTemporalData } =
      template as WebChartBarChartSeries;
    const { lineSmoothed, showArea, markerVisible, markerSymbol } =
      template as WebChartLineChartSeries;
    const { innerRadius, startAngle, endAngle } =
      template as WebChartPieChartSeries;
    defaultSerie = {
      type,
      dataLabels,
      binTemporalData,
...
const getSeriesProps = (
  ...
  const binTemporalData = (serie as unknown as WebChartLineChartSeries)
    ?.binTemporalData;
  const temporalBinning = (serie as unknown as WebChartLineChartSeries)
    ?.temporalBinning;
...
export const createSplitBySerieFromSeries = (
  ...
  const seriesValues = splitByValues.map((splitByValue, index) => {
    let serie = propSeries.find((serie) => {
      ...
      return seriesSplitByValue === splitByValue;
    });
    if (!serie) {
      let splitByProps = {
        propSeries,
...
export const createByFieldSeries = ({
  ...
  serie.x = x;
  (serie as any).y = y;
  // Use the provided name instead of the field name to avoid "OBJECTID" showing up
  serie.name = name || x || y;
  // Use x (category field) as ID for better identification instead of y (aggregation field)
  serie.id = x || y;
  return [serie];
};
...
export const createHistogramQuery = (
  x,
  pageSize?: number,
): FeatureLayerQueryParams => {
  const outFields = [];
...
export const createScatterPlotSeries = (
  { x, y, propSeries },
  dataSourceId?: string,
): WebChartSeries[] => {
  const seriesProps = propSeries[0];
...
export const createScatterPlotQuery = (
  { x, y },
  pageSize?: number,
): FeatureLayerQueryParams => {
  const outFields = [];
...
export const createGaugeOutStatisticDefinition = ({
  statisticType,
  numericField,
}): StatisticDefinition => {
...
const convertArrayToLimitedString = (
  input: string[] | ImmutableArray<string>,
) => {
...
export const getChartTitle = (
  ...
  if (isSerialSeries(seriesType) || seriesType === "pieSeries") {
    ...
    const statisticLabel =
      statistic === "no_aggregation"
        ? statistic
        : translate(statisticTranslation);
    ...
    if (categoryType === CategoryType.ByGroup) {
      ...
      if (splitByField) {
        if (statistic === "no_aggregation") {
          title = translate("numberFieldByCategoryFieldBySplitByField", {
            numberField: numberFieldsLabels[0],
            categoryField: categoryFieldLabel,
            splitByField: splitByFieldLabel,
          });
        } else {
          if (statistic === "count") {
            title = translate("countByCategoryFieldBySplitByField", {
              categoryField: categoryFieldLabel,
              splitByField: splitByFieldLabel,
            });
          } else {
        ...
        if (statistic === "no_aggregation") {
          title = translate("numberFieldByCategoryField", {
            numberField: convertArrayToLimitedString(numberFieldsLabels),
            categoryField: categoryFieldLabel,
          });
        } else {
          if (statistic === "count") {
            title = translate("countByCategoryField", {
              categoryField: categoryFieldLabel,
              splitByField: splitByFieldLabel,
            });
          } else {
            title = translate("statisticOfNumberFieldByCategoryField", {
              statistic: statisticLabel,
              numberField: convertArrayToLimitedString(numberFieldsLabels),
              categoryField: categoryFieldLabel,
            });
      ...
      title = translate("statisticOfNumberField", {
        statistic: statisticLabel,
        numberField: convertArrayToLimitedString(numberFieldsLabels),
      });
    }
    ...
    const numberFieldsLabels = getFieldsAlias(numberFields, dataSourceId);
    title = translate("xFieldAndyField", {
      xField: numberFieldsLabels[0],
      yField: numberFieldsLabels[1],
    });
  } else if (seriesType === "histogramSeries") {
    const numberFields = query.outFields;
    const numberFieldsLabels = getFieldsAlias(numberFields, dataSourceId);
    title = translate("distributionOfNumberField", {
      numberField: numberFieldsLabels[0],
    });
  }
  return title;
...
export const getChartYLabel = (
  ...
  if (isSerialSeries(seriesType) || seriesType === "pieSeries") {
    ...
    if (categoryType === CategoryType.ByGroup) {
      if (statistic === "no_aggregation") {
        label =
          numberFieldsLabels?.length === 1
            ? numberFieldsLabels[0]
            : translate("values");
      } else {
        if (statistic === "count") {
          ...
          label =
            numberFieldsLabels?.length === 1
              ? translate("statisticOfNumberField", {
                  statistic: statisticLabel,
                  numberField: numberFieldsLabels[0],
                })
              : translate("statisticOfValues", { statistic: statisticLabel });
        }
      }
    } else if (categoryType === CategoryType.ByField) {
...
export const useDataSourceFeatureLayer = <T = any>(dataSourceId: string): T => {
  ...
  const pendingPromise =
    React.useRef<cancelablePromise.CancelablePromise>(null);
  ...
  const mainDataSourceId =
    DataSourceManager.getInstance()
      ...
      ?.getMainDataSource().id ?? dataSourceId;
  const mainSourceVersion = useSelector(
    (state: IMState) =>
      state.dataSourcesInfo?.[mainDataSourceId]?.sourceVersion,
  );
  const sourceVersion = useSelector(
    (state: IMState) => state.dataSourcesInfo?.[dataSourceId]?.sourceVersion,
  );
  const needRefresh = useSelector(
    (state: IMState) => state.dataSourcesInfo?.[dataSourceId]?.needRefresh,
  );
  ...
  React.useEffect(() => {
    ...
    if (
      (dataSource as DataSource).type === DataSourceTypes.SceneLayer ||
      (dataSource as DataSource).type ===
    ...
    pendingPromise.current = cancelable(
